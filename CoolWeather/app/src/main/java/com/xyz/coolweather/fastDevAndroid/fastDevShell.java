...
public class fastDevShell {
    ...
    private static final String EXIT = "exit" + fastDevShell.EOL;
            ...
            try {
                ...
                if((line = reader.readLine()) != null) {
                    ...
                    while((line = reader.readLine()) != null) {
                        mBuffer.append(fastDevShell.EOL).append(line);
                    }
                }
            } catch(IOException e) {
                e.printStackTrace();
    ...
    private static String suExec(String cmd) throws ShellException {
        try {
            ...
            shell.writeBytes(cmd + fastDevShell.EOL);
            shell.flush();
            shell.writeBytes(fastDevShell.EXIT);
            shell.flush();
            proc.waitFor();
            return buffer.getOutput();
    ...
    private static boolean isRootUid() throws ShellException {
        for(UID_COMMAND uid : UID_COMMAND.values()) {
            String output = fastDevShell.sudo(uid.getCommand());
    ...
    /**
     * Sets the shell's {@link fastDevShell.OUTPUT output stream}.  Default value is STDOUT.
     *
     * @param ostream The output Stream to read shell from.
     */
    synchronized public static void setOutputStream(fastDevShell.OUTPUT ostream) {
        sOStream = ostream;
    }
    ...
    synchronized public static String sudo(String cmd) throws ShellException {
        if(fastDevShell.su()) {
            return fastDevShell.suExec(cmd);
        } else {
            return null;
        }
    ...
    synchronized public static String exec(String cmd) throws ShellException {
        return fastDevShell.nativeExec(cmd);
    }
}
